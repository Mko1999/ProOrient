...
export default function Hexagon({ norm, code }) {
  ...
  return (
    <svg viewBox="0 0 260 250" style={{ width: "100%", maxWidth: 260, height: "auto" }}>
      <polygon points={frame} fill="none" stroke="rgba(255,255,255,.12)" strokeWidth="1.5" />
      {pts.map((p, i) => (
        <line key={i} x1={cx} y1={cy} x2={p.x} y2={p.y} stroke="rgba(255,255,255,.06)" strokeWidth="1" />
      ))}
      <polygon points={profile} fill="rgba(232,176,75,.20)" stroke="#e8b04b" strokeWidth="2" strokeLinejoin="round" />
      {pts.map((p) => {
        ...
        return (
          <g key={p.t}>
            <circle cx={p.x} cy={p.y} r={lead ? 7 : 5} fill={TYPES[p.t].color} stroke="#10131a" strokeWidth="2" />
            <text x={lx} y={ly} textAnchor="middle" dominantBaseline="middle"
              fontWeight="900" fontSize={lead ? 16 : 13} fill={lead ? TYPES[p.t].color : "#9aa3b2"}>
              {p.t}
            </text>
          </g>
        );
