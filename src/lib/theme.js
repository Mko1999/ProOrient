...
export const S = {
  ...
  eyebrow: {
    display: "inline-flex", alignItems: "center", gap: 9, fontSize: 12.5, letterSpacing: "1.5px",
    textTransform: "uppercase", color: C.accent, fontWeight: 600, marginBottom: 22,
  },
  h1: {
    fontFamily: "Georgia, serif", fontWeight: 900, lineHeight: 1.07,
    fontSize: "clamp(32px,6vw,58px)", letterSpacing: "-.5px", marginBottom: 22,
  },
  grad: {
    background: "linear-gradient(100deg,#e8b04b,#f3d99a 40%,#5b8def)",
    WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent",
  },
  lead: { fontSize: "clamp(15px,2vw,18px)", color: C.muted, maxWidth: 620, marginBottom: 36 },
  h3: { fontFamily: "Georgia, serif", fontSize: 22, marginBottom: 18, fontWeight: 700 },
  body: { fontSize: "clamp(14px,1.6vw,16px)", color: "#aeb6c4", maxWidth: 760, marginBottom: 16, lineHeight: 1.7 },

  pageHead: { padding: "clamp(40px,7vw,68px) 0 10px" },
  section: { marginTop: 26 },

  typeGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 14 },
  badge: {
    width: 42, height: 42, borderRadius: 11, display: "grid", placeItems: "center",
    fontFamily: "Georgia, serif", fontWeight: 900, fontSize: 20, color: "#10131a", marginBottom: 12,
  },
  tcardH: { fontSize: 15.5, marginBottom: 4, fontFamily: "Georgia, serif" },
  tcardP: { fontSize: 12.5, color: C.muted, lineHeight: 1.45 },
  tcardFields: { fontSize: 11.5, color: "#7e8696", lineHeight: 1.45, marginTop: 10 },
  traitList: { listStyle: "none", padding: 0, margin: "12px 0 0" },
  traitItem: { fontSize: 12, color: "#c2cad6", display: "flex", alignItems: "center", gap: 8, padding: "3px 0" },
  traitDot: { width: 6, height: 6, borderRadius: 99, flex: "0 0 auto" },

  metaStrip: {
    marginTop: 40, display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))",
    gap: 1, background: C.line, border: `1px solid ${C.line}`, borderRadius: 16, overflow: "hidden",
  },
  metaCell: { background: C.panel, padding: 22 },
  metaB: { fontFamily: "Georgia, serif", fontSize: 30, display: "block", color: C.accent },
  metaS: { fontSize: 13, color: C.muted },

  tableWrap: { overflowX: "auto", border: `1px solid ${C.line}`, borderRadius: 14, marginTop: 8 },
  table: { width: "100%", borderCollapse: "collapse", fontSize: 13, minWidth: 560 },
  th: {
    textAlign: "left", padding: "13px 16px", color: C.muted, fontWeight: 600, fontSize: 11.5,
    textTransform: "uppercase", letterSpacing: ".6px", background: C.panel2,
    borderBottom: `1px solid ${C.line}`, whiteSpace: "nowrap",
  },
  td: { padding: "13px 16px", borderBottom: "1px solid rgba(255,255,255,.06)", color: "#c2cad6", whiteSpace: "nowrap" },
  trPrimary: { background: "rgba(232,176,75,.07)" },

  archGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 14, marginTop: 8 },
  archTier: { fontSize: 11, color: C.accent, fontWeight: 700, letterSpacing: ".8px", textTransform: "uppercase", marginBottom: 8 },
  stageGrid: { display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 14, marginTop: 8 },
  stageNum: {
    flex: "0 0 auto", width: 36, height: 36, borderRadius: 10, display: "grid", placeItems: "center",
    background: "linear-gradient(180deg,#f0bd5f,#e8b04b)", color: "#241803", fontWeight: 900, fontFamily: "Georgia, serif", fontSize: 17,
  },
  code: {
    fontFamily: "ui-monospace, Menlo, monospace", fontSize: 11.5, color: "#8fd0ff",
    background: "#11151c", border: `1px solid ${C.line}`, borderRadius: 6, padding: "3px 8px", display: "inline-block",
  },

  closeCta: {
    marginTop: 56, textAlign: "center", padding: "44px 24px", borderRadius: 18,
    background: "linear-gradient(180deg,#181c25,#13161d)", border: `1px solid ${C.line}`,
  },
  foot: { marginTop: 70, padding: "30px 0 50px", borderTop: `1px solid ${C.line}`, color: C.muted, fontSize: 13, textAlign: "center" },
};
